/*
  ...
  A variant of Adafruit's fontconvert utility. This version expects
  a string with characters to convert, thus keeping the font-size small:

  ./fc_subset fontfile size 0123456789+-°ChPa%

  To use the font, you first have to map your char to the position
...
int main(int argc, char *argv[]) {
  int               i, j, err, size, bitmapOffset = 0, x, y, byte;
  char              *fontName, c, *ptr, *subset;
  FT_Library         library;
  FT_Face            face;
  FT_Glyph           glyph;
  FT_Bitmap         *bitmap;
  ...
  //   fontconvert filename size stringWithChars 
  // Unless overridden, default first and last chars are
  // ' ' (space) and '~', respectively

  if(argc != 4) {
    fprintf(stderr, "Usage: %s fontfile size stringWithChars\n",
            argv[0]);
    return 1;
  }
  size   = atoi(argv[2]);
  ...
  if((!(fontName = malloc(strlen(ptr) + 24))) ||
     (!(table = (GFXglyph *)malloc((strlen(subset)) *
                                   sizeof(GFXglyph))))) {
  ...
  // Insert font size and "_sub"-prefix.  fontName was alloc'd w/extra
  // space to allow this, we're not sprintfing into Forbidden Zone.
  sprintf(ptr, "%dpt_sub", size);
  ...
  for(i=0, j=0; i<strlen(subset); i++, j++) {
    // extract char from subset
    c = subset[i];

    // MONO renderer provides clean image with perfect crop
    // (no wasted pixels) via bitmap struct.
    if((err = FT_Load_Char(face, c, FT_LOAD_TARGET_MONO))) {
  ...
  if (face->size->metrics.height == 0) {
    ...
    printf("  0x%02X, 0x%02lX, %d };\n\n",
           0, strlen(subset), table[0].height);
  } else {
    printf("  0x%02X, 0x%02lX, %ld };\n\n",
           0, strlen(subset), face->size->metrics.height >> 6);
  }
